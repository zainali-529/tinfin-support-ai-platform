...
import { AtSignIcon, OctagonXIcon, LockIcon, BuildingIcon, UserIcon } from 'lucide-react'
...
function SignupForm() {
  ...
  async function handleSignup(e: React.FormEvent) {
    ...
    const { error } = await supabase.auth.signUp({
      ...
      password,
      options: {
        // Only pass org_name when the user is creating their own workspace
        data: isFromInvite ? { from_invite: true } : { org_name: orgName },
    ...
    <>
      <div className="flex flex-col space-y-1">
        <h1 className="font-bold text-2xl tracking-wide">
          {isFromInvite ? 'Create your account' : 'Join Now!'}
        </h1>
      ...
      {isFromInvite && (
        <div className="flex items-center gap-2 rounded-xl border border-primary/20 bg-primary/5 px-3 py-2.5">
          ...
          <p className="text-xs text-muted-foreground">
            You're joining an organization via invite. No workspace setup needed.
          </p>
        </div>
      )}
      ...
      <div className="space-y-4">
        <form onSubmit={handleSignup} className="space-y-4">
          <div className="space-y-2">
            {/* Only show org name field when NOT from invite */}
            {!isFromInvite && (
              <InputGroup>
                <InputGroupInput
